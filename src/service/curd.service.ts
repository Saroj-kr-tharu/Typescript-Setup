...
class CrudService<T extends PrismaDelegate > {
    ...
    constructor(repo: CrudRepo<T>) {
     ...
     async createService(data: Parameters<T['create']>[0] extends { data: infer D } ? D : never) {
        try {
    ...
    async updateService(
        where: Parameters<T['findUnique']>[0] extends { where: infer W } ? W : never,
   ...
   async deleteService(where: Parameters<T['findUnique']>[0] extends { where: infer W } ? W : never) {
    try {
    ...
    async getAllService(
     ...
     async getByIdService(where: Parameters<T['findUnique']>[0] extends { where: infer W } ? W : never) {
        try {
