...
class CrudRepo<
  ...
  WhereUniqueInput = Parameters<T['findUnique']>[0] extends { where: infer W }
    ? W
    : never,
  WhereInput = Parameters<T['findFirst']>[0] extends { where?: infer W }
    ? W
    : never,
> {
  protected model: T;
  ...
  update(
    ...
    data: UpdateArgs extends { data: infer D } ? D : never,
  ) {
